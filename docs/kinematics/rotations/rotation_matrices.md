...
Special orthogonal group $SO(3)$ can be expressed by rotation matrices as:

$$
SO(3) = \left\{ \mathbf{R} \in \mathbb{R}^{3 \times 3} \ | \ \mathbf{R} \mathbf{R}^T = \mathbf{R}^T \mathbf{R} = \mathbf{I}, \ \text{det}(\mathbf{R}) = 1 \right\}.
$$

Every matrix $\mathbf{R} \in SO(3)$ is a 3D rotation matrix and is a distance preserving linear transformation. Consider an arbitrary vector $\mathbf{x} \in \mathbb{R}^3$ and two frames $F_\alpha$ and $F_\beta$ with a common origin. Then we have:

$$
\mathbf{x}^{\beta} = \mathbf{R}^{\beta}_{\alpha} \mathbf{x}^{\alpha},
$$

where the superscript is the frame where $\mathbf{x}$ is resolved in and $\mathbf{R}^{\beta}_{\alpha}$ is the rotation matrix from $F_\alpha$ to $F_\beta$ defined as:
...
where $\theta_{\beta_i \alpha_i}$ is the angle between the $i$'th axis of frame $F_\alpha$ and $F_\beta$. The two frames are defined by set of orthonormal base vectors:
...
??? cnote "Proof"
    ...
    Vector $\mathbf{x}$ can be expressed in each frame as:
...
The inverse or transpose of $\mathbf{R}^{\alpha}_{\beta}$ represents the inverse transformation:
...
Rotation matrix can be used to transform $\mathbf{M}$ to which specific resolving axes apply. Substituting one to another in equations ($\ref{bma1}$) and ($\ref{bma2}$):
...
The two rotations are inverse of each other and are interchangeable. Consider an arbitrary vector $\mathbf{p}$. If the object frame $F_\alpha$ has been rotated from the reference frame $F_\beta$ through a rotation about one basis vector (either 3, 2, or 1) by an angle (either $\psi$, $\theta$, or $\phi$), then the principal rotation matrices (from $F_\beta$ to $F_\alpha$ denoted as $\mathbf{R}^\alpha_\beta$) are given as:
